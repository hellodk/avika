...
- **Images:** Gateway and frontend use `global.imageRegistry` (see [CONTAINER_REGISTRY.md](../../docs/CONTAINER_REGISTRY.md)) and `repository`/`tag` from `values.yaml` (e.g. `docker.io/hellodk/avika-gateway:latest`) with `pullPolicy: Always`.
