...
import { Cpu, FolderKanban, Activity, ArrowRight, X, Zap } from "lucide-react";
...
const steps: StepDef[] = [
    ...
    },
    {
        ...
        description:
            "Install the Avika agent on your NGINX server. It will connect to this deployment's gateway (not a public installer domain). If the agent runs elsewhere, use a hostname or IP that host can reach.",
...
const INSTALL_FALLBACK =
    "curl -fsSL http://<GATEWAY_HOST>:5021/updates/deploy-agent.sh | sudo bash";

export function OnboardingWizard() {
    const [open, setOpen] = useState(false);
    const [step, setStep] = useState(0);
    const [installOneLiner, setInstallOneLiner] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const ar = await fetch(apiUrl("/api/servers"), { credentials: "include" });
                if (ar.ok) {
                    const jd = (await ar.json()) as { agents?: unknown[] };
                    const agents = Array.isArray(jd?.agents) ? jd.agents : [];
                    if (agents.length > 0) {
            ...
            try {
                ...
                if (cr.ok) {
                    const cfg = (await cr.json()) as {
                        gateway?: { httpUrl?: string; host?: string; grpcPort?: string };
                    };
                    const httpUrl = cfg?.gateway?.httpUrl;
                    if (httpUrl) {
                        const base = httpUrl.replace(/\/$/, "");
                        setInstallOneLiner(`curl -fsSL ${base}/updates/deploy-agent.sh | sudo bash`);
                    }
                }
            } catch {
                /* use fallback string */
    ...
    const agentInstallCode = useMemo(() => {
        if (current.id !== "agent") return null;
        return installOneLiner ?? INSTALL_FALLBACK;
    }, [current.id, installOneLiner]);
        ...
        <Dialog open={open} onOpenChange={(v) => { if (!v) handleComplete(); }}>
            <DialogContent className="sm:max-w-md p-0 overflow-hidden" style={{ background: "rgb(var(--theme-surface))", borderColor: "rgb(var(--theme-border))" }}>
                ...
                <div className="p-6 pt-4">
                    ...
                    {current.hint && agentInstallCode && (
                        <p className="text-xs mb-2" style={{ color: "rgb(var(--theme-text-muted))" }}>
                            {current.hint}
                        </p>
                    )}
                    {agentInstallCode && (
                        <div className="mb-4 rounded-lg p-3 font-mono text-xs break-all whitespace-pre-wrap" style={{ background: "rgb(var(--theme-background))", borderColor: "rgb(var(--theme-border))", border: "1px solid" }}>
                            <p style={{ color: "rgb(var(--theme-text))" }}>{agentInstallCode}</p>
                        </div>
                    )}
